...
if st.button('Predict'):
    ...
    if prediction[0] <0.5:
        risk_level = 'High'
    elif 0.5 <=prediction[0] <= 0.7:
        risk_level = 'Medium'
    else:
        risk_level = 'low'
    st.write(f"Heart Attack Risk Level: {risk_level}")

    # Collect values for the bar chart
    ...
    bar_fig = go.Figure(data=[go.Bar(
        x=bar_chart_labels,
        y=bar_chart_values,
        marker=dict(color=['red', 'blue', 'green', 'orange', 'purple', 'yellow'])
    )])
    ...
    # Display line chart for heart attack risk
    st.write("### Predicted Heart Attack Risk Over Time ")
    st.write("if\nrisk<0.5 high\nrisk=0.5-0.7 medium\nrisk>0.7 low ")
    line_fig = go.Figure(data=go.Scatter(
        x=['Current Prediction'],  # Placeholder for time points
        y=[prediction[0]],
        mode='lines+markers',
    ...
    line_fig.update_layout(
        ...
        yaxis=dict(title='Heart Attack Risk'),
        title='Predicted Heart Attack Risk Over Time'
    )
    ...
    if recommendations:
        ...
        for rec in recommendations:
            st.write(f"- {rec}")
 #cd '.\Heart Disease AI\'cd '.\Heart Disease AI\'